...
if ($id <= 0 || $titol === '') {
    header('Location: ../controller/modificarPage.controller.php?id=' . $id . '&error=' . urlencode('Datos inválidos.'));
    exit;
}
...
if ($ok) {
    header('Location: ../index.php?ok=' . urlencode('Pokémon modificado correctamente'));
    exit;
}
header('Location: ../controller/modificarPage.controller.php?id=' . $id . '&error=' . urlencode('No se ha podido modificar. Inténtalo de nuevo.'));
exit;
